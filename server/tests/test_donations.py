...
def test_throttling(client, donation_request):
    ...
    data = {
        ...
        'items': [item.id for item in donation_request.request_items.all()],
    ...
    assert Request.objects.count() == 13
    ...
    assert response.status_code == status.HTTP_201_CREATED
    assert Request.objects.count() == 14
    ...
    assert response.status_code == status.HTTP_201_CREATED
    assert Request.objects.count() == 15
