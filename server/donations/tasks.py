...
from donations.messages import (
    RequestReceivedSMS, EMAIL_MESSAGE_TEMPLATES,
...
def send_request_confirmation_message(instance):
    if instance.email:
        send_email_message(
            ...
            context={
                'items': [
                    item.id for item in instance.request_items.all()
                ],
                'neighborhood': instance.neighborhood.name,
            }
        )
    elif instance.phone:
        RequestReceivedSMS(instance=instance).send()


def send_donor_appointment_confirmation_messages(instance):
